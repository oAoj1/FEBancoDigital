...
export default function TabelaPessoal(){

    const [gastos,setGastos] = useState<any>([])
    const [somasPessoal,setSomasPessoal] = useState<any>([])

    useEffect(() => {
        async function lerGastos(){
            const response = await api.get('/gasto/filtrar?classificacao=pessoal')
            const data = response.data

            setGastos(data)
        }

        async function lerSomasPessoal(){
            const response = await api.get('/somasgerais')
            const data = response.data

            setSomasPessoal(data)
        }
